...
```{r}
#2a. make a lagos spatial dataset that has the total number of counts per site. Presumably we are looking for states that have the most chla and secchi data..?

# Start with the lagos spatial dataset
chla_secchi_200 <- chla_secchi %>%
  group_by(lagoslakeid) %>%
  mutate(count = n()) %>%
